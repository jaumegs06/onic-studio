...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    const urlParts = req.url?.split('/projects/') || [];
    const idPart = urlParts[1]?.split('?')[0]; // Remove query params if any
    const id = idPart && idPart !== '' ? idPart : null;

    console.log('[Projects API] Method:', req.method, 'ID:', id, 'URL:', req.url);
